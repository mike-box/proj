...
taskset 1 ${HSBENCH_BIN} -e pcre/snort_literals -c corpora/alexa200.db -N -T 10

echo "\n*** Snort literals against Gutenberg text, block mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_literals -c corpora/gutenberg.db -N -T 10
echo

echo "\n*** Snort literals against news text, block mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_literals -c corpora/news.db -N -T 10
echo

echo "\n*** Snort PCREs against alexa200 text, block mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_pcres -c corpora/alexa200.db -N -T 10
echo

echo "\n*** Snort PCREs against Gutenberg text, block mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_pcres -c corpora/gutenberg.db -N -T 10
echo

echo "\n*** Snort PCREs against news text, block mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_pcres -c corpora/news.db -N -T 10
echo

echo "\n*** Snort literals against HTTP traffic, alexa200 text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_literals -c corpora/alexa200.db -T 10
echo

echo "\n*** Snort literals against HTTP traffic, Gutenberg text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_literals -c corpora/gutenberg.db -T 10
echo

echo "\n*** Snort literals against HTTP traffic, news text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_literals -c corpora/news.db -T 10
echo

echo "\n*** Snort PCREs against HTTP traffic, alexa200 text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_pcres -c corpora/alexa200.db -T 10
echo

echo "\n*** Snort PCREs against HTTP traffic, Gutenberg text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_pcres -c corpora/gutenberg.db -T 10
echo

echo "\n*** Snort PCREs against HTTP traffic, news text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/snort_pcres -c corpora/news.db -T 10
echo

echo "\n*** Teakettle synthetic patterns against alexa200 text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/teakettle_2500 -c corpora/alexa200.db -T 10
echo

echo "\n*** Teakettle synthetic patterns against Gutenberg text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/teakettle_2500 -c corpora/gutenberg.db -T 10
echo

echo "\n*** Teakettle synthetic patterns against news text, streaming mode.\n"
taskset 1 ${HSBENCH_BIN} -e pcre/teakettle_2500 -c corpora/news.db -T 10
echo